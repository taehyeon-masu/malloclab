...
inline static void masu_add_free_delay(void *bp);
inline static void masu_remove_free_delay(void *bp);
inline static void masu_delay_class_coalesce();
...
void *masu_realloc(void *ptr, size_t size){
	...
	if (PREV_BLKP(mem_brk) == ptr){next_alloc = 1;}
